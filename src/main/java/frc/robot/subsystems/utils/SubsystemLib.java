...
public abstract class SubsystemLib extends SubsystemBase {
  @AutoLogOutput protected boolean attached = false;
  ...
  protected abstract Config
      setConfig(); // abstract method that forces all subclasses to define config
  ...
  public void setMMPosition(double position, int slot) {
    if (attached) {
      MotionMagicVoltage mm = config.mmPositionVoltageSlot.withSlot(slot).withPosition(position);
      motor.setControl(mm);
    }
  }
  ...
  public void setPercentOutput(double percent) {
    if (attached) {
      VoltageOut output = config.voltageControl.withOutput(12 * percent);
      motor.setControl(output);
    }
  }
  ...
  public static class Config {
    ...
    public void applyTalonConfig(TalonFX talon) {
      ...
      if (!result.isOK()) {
        DriverStation.reportWarning(
            "Could not apply config changes to motor!",
            false); // Maybe this will help fix the chain problem?
      }
    }

    public void isClockwise(
        boolean
            clockwise) { // ALWAYS test this! We do not want chains snapping. Run the motros before
      // we put the mechanism on
      if (!clockwise) {
    ...
    public void configGearRatio(
        double gearRatio) { // find the number of gearboxes here for an accurate number of shaft
      // rotations
    ...
    public void configNeutralBrakeMode(
        boolean
            isInBrake) { // If we have a mechanims that can not hold itself set the neutral mode to
      // Brake(true)
      if (isInBrake) {
    ...
    /**
     * Defaults to slot 0 //Some other teams have a complicated slot configuration but I just have
     * this simple default until I learn more
     *
    ...
    public void configPIDGains(
        double kP, double kI, double kD) { // Super simple functions to set the PID gains.
      configPID(kP, kI, kD);
    }
