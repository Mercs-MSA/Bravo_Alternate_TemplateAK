...
public class ApriltagLimelightVision {
    ...
    public void updateVisionEstimates(Rotation2d orientation) {
        ...
        LimelightHelpers.SetRobotOrientation(Constants.VisionConstants.limelightPoseEstimatorName, orientation.getDegrees(), 0, 0, 0, 0, 0);

        PoseEstimate pos;
     
            pos = LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(Constants.VisionConstants.limelightPoseEstimatorName);
    ...
    public void applyVisionToEstimator() {
        if (bestEstimate != null) {
            m_robotContainer.drive.addVisionMeasurement(bestEstimate.pose, Utils.fpgaToCurrentTime(bestEstimate.timestampSeconds), Constants.VisionConstants.visionStdDevs);
        }
